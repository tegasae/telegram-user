...
class AlmostRepository(AbstractRepository):
    ...
    def get_senders(self) -> list[Sender]:
        ...
        for s in self.src:
            if (s['confirmed']==2) and (s['status'] & 8):
                senders.append(Sender(id=s['id'],name=s['name'],telegram_id=s['telegram_id'],api_id=0,api_hash=""))
        return senders
    ...
    def get_sender(self, sender_id: int) -> Sender:
        for s in self.src:
            if s['id']==sender_id:
                return Sender(id=s['id'],name=s['name'],telegram_id=s['telegram_id'],api_id=0,api_hash="")
